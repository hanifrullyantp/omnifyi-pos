...
const initialState: CmsState = {
  ...
  features: {
    ...
    items: [
      { title: 'Kasir On/Offline', desc: 'Jualan tak pernah terhenti meski tanpa sinyal internet.' },
      { title: 'Akuntansi', desc: 'Pembukuan otomatis tanpa ribet menghitung manual.' },
      { title: 'Aplikasi CRM', desc: 'Pahami pelanggan Anda dan buat mereka kembali berbelanja.' },
      { title: 'Dashboard Realtime', desc: 'Pantau omzet dan profit secara langsung dari HP Anda.' },
      { title: 'Karyawan', desc: 'Kontrol akses kasir dan pantau performa tim setiap saat.' },
      { title: 'Inventori', desc: 'Stok barang selalu akurat dan tersinkron dengan penjualan.' },
      { title: 'Analisa Bisnis', desc: 'Dapatkan insight cerdas untuk keputusan bisnis yang tepat.' },
      { title: 'Order Online', desc: 'Terima pesanan dari pelanggan secara digital.' },
      { title: 'Business Plan', desc: 'Rencanakan pertumbuhan usaha dengan data yang solid.' }
    ]
  },
  steps: {
...
export const CmsProvider = ({ children }: { children: ReactNode }) => {
  const [data, setData] = useState<CmsState>(() => {
    const saved = localStorage.getItem('omnifyi_cms_data');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('omnifyi_cms_data', JSON.stringify(data));
  }, [data]);
