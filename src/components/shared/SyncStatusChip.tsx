...
export function SyncStatusChip({ className }: { className?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={() => {
        if (online && pending > 0 && !isSyncing) void processSyncQueue();
      }}
      className={cn(
      ...
      title="Status koneksi (local-first). Klik untuk sync manual saat online."
    >
      <span className="text-base leading-none">{!online ? '🔴' : pending > 0 ? '🟡' : '🟢'}</span>
      {label}
    </button>
  );
}
